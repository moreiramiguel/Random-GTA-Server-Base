...
RegisterKeyMapping('mdt', 'Abrir MDT (Policia)', 'keyboard', 'k')
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        ...
        QBCore.Functions.Notify("Não podes fazer isso!", "error")
    end
end, false)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS ativado!', 'success')
end)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                QBCore.Functions.Notify('Veículo não encontrado!', 'error')
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        QBCore.Functions.Notify("Definiste a frequência do rádio para "..radio..".", "success")
    else
        QBCore.Functions.Notify("Estação inválida(colona um número)", "error")
    end
end)
...
RegisterNetEvent('mdt:client:sig100', function(radio, type)
    ...
    if AllowedJob(job) and duty == 1 then
        if type == true then
            exports['erp_notifications']:PersistentAlert("Começar", "signall100-"..radio, "inform", "Radio "..radio.." is currently signal 100!")
        end
    end
    if not type then
        exports['erp_notifications']:PersistentAlert("Acabar", "signall100-"..radio)
    end
end)
