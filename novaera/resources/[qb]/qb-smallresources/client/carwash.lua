...
RegisterNetEvent('qb-carwash:client:washCar', function()
    ...
    washingVehicle = true
    QBCore.Functions.Progressbar("search_cabin", "O veículo está a ser limpo ..", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Limpeza cancelada ..", "error")
        washingVehicle = false
    end)
end)
                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], '~g~E~w~ - Limpar veículo ($'..Config.DefaultPrice..')')
                                if IsControlJustPressed(0, 38) then
                                    if dirtLevel > Config.DirtLevel then
                                        TriggerServerEvent('qb-carwash:server:washCar')
                                    else
                                        QBCore.Functions.Notify("O veículo não está sujo", 'error')
                                    end
                                end
                            else
                                DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], 'A limpeza de veículos não está disponível ..')
                            end
                        end
                    end
                end
